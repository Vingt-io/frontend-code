            ...
            <div className="container mx-auto px-4" id="WhitePaperSection">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {productsData.map((product, index) => (
                        <div key={index} className="p-6 bg-[#13111a] rounded-lg shadow-lg">
                            <div className='flex justify-between items-center'>
                            <Link href={product.link} target="_blank" className="flex items-center gap-2">
                                {product.image && <Image src={product.image} alt="icon" className="w-auto h-10 transition-transform duration-300 hover:scale-110" />}
                                <h3 className="text-lg font-semibold hover:text-blue-500">{product.title}</h3>
                            </Link>
                ...
                <div className="overflow-x-auto mt-6">
                    <table className="w-full ">
                        <thead>
                            <tr className="bg-primaryblue border-b border-[#e0e0e0]">
                        ...
                        <tbody>
                            {product_indextoken.map((product, index) => (
                                <tr key={index} className="text-center  bg-[#13111a]">
                                    <td className="p-3  flex items-center gap-4 ">
                                        <Image src={product.image} alt="Product Image" className="w-auto h-10" />
                                    ...
                                    <td className="p-3">{product.symbol}</td>
                                    <td className="p-3">{product.market_cap}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
